...
void Player::update(float delay)
{
	...
	// ca c pour le reload complet du shotty...jme demande ben ou a part ici ca devrait aller......dans shoot c appel� seulement quand ya un mouse HOLD....faq c la meilleur place que j'ai trouver la
	if (weapon)
	{
		if (weapon->fullReload && weapon->weaponID == WEAPON_SHOTGUN)
		{
			if (weapon->shotInc > 0)
			{
				// si ca fait un sizieme du temps allou� pour reloader
...
#ifndef DEDICATED_SERVER
	// On check si on change de nom (on fait �a uniquement si on est � la fin d'un round (tout le temps en fin de compte))
	if (isThisPlayer)
	{
	...
	if (status == PLAYER_STATUS_ALIVE)
	{
		...
		else
		{
			...
			if (isThisPlayer)
			{
				// Bon, on envoit cette position aux autres joueurs, la vitesse d'envoit c'est d�pendant de son ping
				sendPosFrame++;
				int clampedPing = avgPing;
				if (gameVar.sv_maxSendInterval > 0 && clampedPing > gameVar.sv_maxSendInterval)
					clampedPing = gameVar.sv_maxSendInterval;
				if (sendPosFrame >= clampedPing && sendPosFrame >= gameVar.sv_minSendInterval && status == PLAYER_STATUS_ALIVE)
				{
		...
		status == PLAYER_STATUS_DEAD && timeToSpawn >= 0)
	{
		...
		if (timeToSpawn <= 0)
		{
			// On ne veut pas afficher des n�gatif ;)
			timeToSpawn = 0;
...
#ifndef DEDICATED_SERVER
//
// Pour le controller (�a c'est client side only, on ne g�re pas le mouvement des autres players comme �a)
//
void Player::controlIt(float delay)
{
	// On g�re les inputs

	// Si on est en mode scope (FPS), on tourne la t�te avec la mouse
	if (scopeMode)
	{
	...
	// Si on est en mode scope, on se d�place dapres l'orientation local
	if (scopeMode)
	{
	...
	if (dkiGetState(gameVar.k_shoot) && initedMouseClic)
	{
		if (weapon && grenadeDelay == 0 && meleeDelay == 0) 
		{
			...
			{
				//--- On shoot une deuxi�me fois pour faire plus de damage en scope mode
				weapon->nbShot = 3;
			}
	...
	// On switch en scope mode (sniper only) (c�tait juste un test � chier �a)
